...
2. 点击 **「开始设计」**。
3. 依次在 2–6 步点击 **「生成本步内容」**，观察中间结构化卡片。
4. 任意卡片**可直接编辑**，右侧预览实时同步。
5. 第 6 步生成支架与质检后，点击 **「生成完整教学设计」**。
6. 右侧出现完整 Markdown，点 **「复制Markdown」** 或 **「下载 .md」**。
...
`step` 枚举：`start` / `standard-analysis` / `goals` / `evidence` / `task-chain` / `support-quality` / `final-lesson`。
...
登录、数据库、用户系统、文件上传、Word/PDF 导出、历史记录、多用户、复杂知识库、多主题模板、复杂动画——
保证**页面能打开、流程能跑、mock 能演示、最终教案能复制、代码不崩**。
...
**右侧第二个 Tab「AI 引导助手」**：嵌入 Dify 智能体 iframe（`https://udify.app/chatbot/hvtT94ljurxSFeyC`），供教师在卡住时追问课标拆解、目标定位、评价证据与任务链设计。仅作辅助追问，**正式成果仍以左侧六步工作台为准**；桌面端 iframe 最小高度 700px、移动端 70vh，仅在该 Tab 激活时加载（`loading="lazy"`）。未接入 bubble 悬浮脚本，避免遮挡工作台按钮。
